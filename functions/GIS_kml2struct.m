function kmlStruct = GIS_kml2struct(kmlFile)
% kmlStruct = kml2struct(kmlFile)
%
% I borrowed this function from James Slegers:
% https://www.mathworks.com/matlabcentral/fileexchange/35642-kml2struct. I
% changed the line which splits coordinates into 3 columns, seemeed to make
% it work...
%
...
for ii = 1:Nos
    ...
    % ML: I have changed this next bit multiple times. Hopefully now stable.  Sometimes it fails here, because we end up with an n*m by 2 matrix (z coordinates not included). I don't quite understand this...
    if coordMat(3)==0
       coordMat = reshape(coordMat,3,m*n/3)'; 
    elseif rem(length(coordMat),2)==0
       coordMat = reshape(coordMat,2,m*n/2)'; 
    else 
       exception=MException('MATLAB:CoordMatError','coordMat size not understood. Are you trying to import 4d coordinates or something!?');
       throw(exception)
    end
