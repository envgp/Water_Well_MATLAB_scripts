function [x y depth stn_id site_code well_depth well_use number_msmts] = calc_water_levels_woduplicates_exportversion(Data,method)
% [x y depth] = calc_water_levels_woduplicates(Data,method) returns
...
% of the two measurements. This script seems to work but I'm not sure where
% the bit that removes NaNs is....!
...
well_use = Data.WellData.well_use(wellrefs);
number_msmts = zeros('like',x); % initiate number_msmts
...
x = x(indicestokeep);
y = y(indicestokeep);
site_code = site_code(indicestokeep);
well_depth = well_depth(indicestokeep);
well_use = well_use(indicestokeep);
